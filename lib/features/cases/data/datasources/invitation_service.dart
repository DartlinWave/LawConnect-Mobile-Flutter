...
class InvitationService {
  ...
  Future<List<Invitation>> fetchInvitationsByCaseId(
    String caseId,
    String token,
  ) async {
    final uri = Uri.parse(
    ...
    if (response.statusCode == HttpStatus.ok) {
      ...
      return invitations
          .map((invitation) => InvitationDto.fromJson(invitation).toDomain())
          .toList();
    } else {
      throw Exception(
        'Failed to fetch invitations: ${response.statusCode} for case $caseId',
      );
    }
  }
}
